 ...
 * Usage:
 *
 *  ./dids_server_image_test <SQL_INFO> <IMAGE_FILENAME>
 *
 *  ./dids_server_image_test "dbname = 'test' user = 'test' connect_timeout = '10'" myTestImage.jpg
 *
...
void usage(FILE *sock_fh, char *appname) {
    ...
    fprintf(sock_fh, "\n");
    fprintf(sock_fh, "  %s \"dbname = 'test' user = 'test' connect_timeout = '10'\" myTestImage.jpg\n", appname);
    fprintf(sock_fh, "\n");
}
...
int main(int argc, char *argv[]) {
    ...
    PicInfo *list = NULL;
    PicInfoAddToList(sock_fh, &list, hlp);
    ...
    ppm_sql_disconnect(sock_fh, psql);
    fprintf(sock_fh, "INFO: dids_server_image_test - All tests passed\n");
    exit(0);
}
